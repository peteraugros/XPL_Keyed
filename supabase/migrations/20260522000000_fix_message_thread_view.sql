-- Fix: derived_tasks_view.message_thread was leaking stale tasks.
--
-- Bug: the latest_msg CTE filtered `WHERE waiting_on = 'TIM'` BEFORE
-- DISTINCT ON, so when Tim replied (new message with waiting_on='KID'),
-- the new row was filtered out and the CTE still found the older kid
-- message as the "latest." Task never cleared from Tim's Home queue
-- even though the conversation had moved on.
--
-- Fix: move the waiting_on filter AFTER the DISTINCT, so the CTE picks
-- the actual latest message per player and the filter excludes it when
-- Tim has already replied.
--
-- No other CTE in this view has the same bug (latest_vod, latest_prep
-- have their filters applied post-CTE; next_stub_slot and unmarked_call
-- don't use the DISTINCT ON pattern). Only message_thread was affected.
