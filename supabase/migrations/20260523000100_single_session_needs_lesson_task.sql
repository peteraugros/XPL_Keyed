-- Add single_session_needs_lesson task to derived_tasks_view.
--
-- Fires when a parent has paid for a $24 single coaching session
-- (subscription.tier='single_lesson', curriculum.curriculum_type=
-- 'single_session', curriculum.status='active') and the slot still
-- has lesson_id NULL — meaning Tim hasn't yet picked or built a
-- lesson based on the parent's intake answer.
--
-- Priority 76 — slots between call_outcome_pending (78) and
-- lesson_authoring_needed (75). Single-session has higher urgency
-- than cycle lesson-stub work because the parent has actively paid
-- + scheduled and the lesson must be ready by call time.
--
-- task_payload exposes the parent's "what to help with" answer
-- (curricula.personalization_note for single_session curricula) so
-- Tim has the signal he needs to pick (or build) the right lesson
-- without leaving Focused Home.
--
-- Body is otherwise the same as the previous view definition — the
-- full CREATE OR REPLACE is unavoidable because Postgres doesn't
-- support appending a single UNION branch to an existing view.
...
-- single_session_needs_lesson. Fires after a $24 single coaching
-- session purchase when the slot's lesson_id is still NULL. Tim
-- needs to pick or build a lesson based on the parent's intake
-- note before the call. Drops automatically when Tim assigns the
-- lesson via the existing /admin lesson-swap UI.
SELECT
