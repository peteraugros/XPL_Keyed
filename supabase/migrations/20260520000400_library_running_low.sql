-- Tim's library is running low. Fires when the count of PUBLISHED
-- lessons is below a threshold (we use 12 = 3 cycles' worth of fresh
-- content as the soft alarm). Tim sees the card on Focused Home and
-- can author more.
--
-- Awareness card: P22 (low priority — not blocking anything today).
-- Single fixed row per coach, dedupes on coach_id. Drops automatically
-- once the published lesson count crosses back over the threshold.
...
-- Library running low. Per-coach awareness when the published lesson
-- count is under 12 (= 3 cycles of fresh content). Auto-renew starts
-- repeating lessons or stub-falling back below that. P22 — low
-- priority because it's a long-horizon problem, not a blocker today.
SELECT
