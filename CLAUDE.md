...
3. **Stripe webhook handler** at `src/app/api/stripe-webhook/route.ts`:
   - `invoice.payment_failed` → set subscription `past_due`, `past_due_started_at`
   - `invoice.paid` → increment `cycle_lessons_delivered`, reset `cycle_cancels_used` when cycle completes
