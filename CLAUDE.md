...
- **$24 single coaching session — smoke test remaining.** Phase 1 + Tim-side surfacing + Phase 2 (parent-portal UX + scheduler + Stage C decline offer) + schedule-before-pay restructure all shipped 2026-05-23 (see Done entries). Open:
  ...
  - **Deno-side Calendly REST cancel in `cron-payment-abandonment`.** When a single-session (or cycle) family abandons after Calendly booking but before paying, the cron nulls `live_call_at` + `live_call_event_id` in our DB after 24hr but doesn't call Calendly's cancellation REST endpoint. Tim sees a phantom event in his Calendly with no DB record and manually cancels. ~30 min to add a Deno-side fetch in the cron mirroring the Node helper `cancelCalendlyEvent`.
