...
5. **Intake flow** — Backend half is **DONE 2026-05-17** (see Done entry "Intake backend complete"): `pending_intake_verifications` migration, `rpc_intake()` SECURITY DEFINER, and the three API routes (`/api/intake/request-verification`, `/intake/verify`, `/api/intake/submit`) all built and tested end-to-end against the local stack. **Remaining work:** Stage A 4-level gamified form at `/intake` (kid-first first name/age/Fortnite/Discord at L1 with inline under-13 parent gate per the locked COPPA Option C; rank/platform/hours at L2; parent name/email at L3; Calendly embed with prefill at L4). LocalStorage auto-save, rarity-colored levels (uncommon→rare→epic→legendary), XP bar, +25 XP floats, confetti on final submit, "Achievement Unlocked" screen, level-up sound (muted by default with toggle). Mobile-first. Verify the Calendly intro-call form (Setup item #4) before wiring the L4 prefill so the `a1`–`a5` field IDs are confirmed.
