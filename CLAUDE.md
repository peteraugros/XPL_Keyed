...
- **$24 single coaching session — Phase 2 only.** Phase 1 + Tim-side surfacing shipped 2026-05-23 (see Done entry "$24 single coaching session shipped end to end" for the locked design + everything that landed). Phase 2 is small follow-up work:
  - **Stage C decline-with-single-session offer** — when Tim picks "Not the right fit" on a trial, the decline email should offer the $24 single session as a step before the free-creator recommendations (graceful downgrade ladder: subscription → single → free creators).
  - **End-to-end smoke test against a fresh email** to validate the webhook-to-magic-link path now that the orphan-parent lookup bug is fixed.
