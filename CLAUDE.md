...
- **$24 single coaching session — smoke test remaining.** Phase 1 + Tim-side surfacing + Phase 2 (parent-portal UX + scheduler + Stage C decline offer) + schedule-before-pay restructure all shipped 2026-05-23 (see Done entries). Open:
  - **End-to-end smoke test against a fresh email** to validate webhook → magic link → /portal single-session dashboard → /portal/sessions Calendly booking → confirmation email, plus exercise the Stage C decline-with-single-session offer from a separate trial family.
