...
1. ~~**Ongoing curriculum approval (cycles 2+).**~~ **RESOLVED 2026-05-22 — different model than the spec said, but functionally equivalent.** Verified by tracing `provisionNextCycle` in `src/lib/lessons/auto-renew.ts` + the auto-renew cron + Stripe webhook chain.
    ...
    **Small follow-up to close the uniform gap:** add a soft confirmation prompt on `/portal` at cycle-2 start — "Your next 4 sessions are predicted at [Sat 2pm × 4]. Confirm or reschedule." One card surface, no blocking. Much cheaper than rebuilding the approval flow as a hard gate. **Status: queued.** Build alongside the next "Calendly auto-booking of uniform predicted times" pass (Open TODO #11) since they're the same problem.
