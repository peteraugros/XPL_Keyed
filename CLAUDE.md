...
6. **Lesson editor is the in-app 7-step planner (2026-05-24).** Video-first lessons: Tim records a video (YouTube/Vimeo/Loom unlisted), the planner produces a beat sheet + glossary + parent translation pair, the published lesson row holds `video_url` and serves through the Sunday email. The slide+audio era is RETIRED — lessons.slides column stays NULL on new rows, the legacy upload form is gone. Path B lean per the May 23 architectural call. AI helpers at every planner step except Review; output is editable suggestions, never auto-applied. See the big Done entry below for the full arc.
...
- ✅ **DONE. Kept because THE APP ICON LIVES IN THREE PLACES AND ONLY TWO ARE
  ...
  and the browser tab favicon is an inline SVG **data URI in `layout.tsx`** that
  no icon file search would ever surface. All three are L now.
  **Re-rasterize the PNG in a BROWSER, never with sharp**: the glyph offset in
  `icon.svg` was measured from a browser render, so a different rasterizer picks
...
- **Heavy parent trust signals.**
  ...
  - **Call recording is RETIRED (2026-05-22).** Originally locked as a heavy trust signal; retired after reviewing the workflow cost (asking a 14yo to start/stop QuickTime + system-audio routing every call is fragile, and the storage/upload/playback infrastructure was unbuilt). The trust posture lives on without it: parent-as-observer rights in every kid's private Discord channel give live-listen capability, which is a stronger signal than after-the-fact playback. Tim's per-lesson `coach_note` lands on `/portal/progress` so every call has a written recap. **Do not reinstate without revisiting trust posture.**
...
- **Separate linked parent + player accounts under one family record.**
  ...
  - Parent Profile: own login, billing, cancel, approval queue, message audit, live observer access to the kid's Discord channel.
...
- "Your controls" panel — Billing / Message audit (all visible but empty in trial state) + Cancel trial link. Empty state intentionally shows what *will* be there so it feels familiar when it fills in. (Call recordings card removed when recording was retired 2026-05-22.)
...
- Parent is invited as observer when Tim creates the channel. **No call recording** — that feature was retired 2026-05-22 (see Account & trust model section above). Parent's trust signals are live observer access + Tim's post-call notes on `/portal/progress`.
- **Server template + channel naming convention + parent-observer role config are NOT documented** anywhere yet. For operator #2 this becomes a real deliverable; for Tim's n=1 it's tribal knowledge.
...
1. ~~**Ongoing curriculum approval (cycles 2+).**~~ **RESOLVED 2026-05-22 — different model than the spec said, but functionally equivalent.** Verified by tracing `provisionNextCycle` in `src/lib/lessons/auto-renew.ts` + the auto-renew cron + Stripe webhook chain.

    **What actually happens at cycle 2+:** the auto-renew cron fires the $56 charge without any approval click. New curriculum is created `status='active'` immediately. Then the flow forks on the *prior* cycle's booking pattern:
    - **Scattered parents** (different weekdays each cycle): `lifecycle_state='SCHEDULING_IN_PROGRESS'`, slots have `live_call_at=NULL`. `/portal/sessions` automatically surfaces the SchedulerWizard. Parent must schedule each session before lessons advance. `cron-scheduling-abandonment` pings them if they ghost. The 60-day ToS refund window does the rest. This is the model Peter described — pay → prompt for dates → no reply → keep money after the policy window.
    - **Uniform parents** (same weekday + time each cycle): `lifecycle_state='ACTIVE'` directly, slots soft-booked at predicted forward times. *No prompt fires.* Lesson materials (slides + audio) still ship every Sunday via `cron-sunday-lesson-delivery`. **But no real Calendly event gets created at the predicted times** (Calendly auto-booking is open TODO #11), so the live call portion silently doesn't happen until the parent confirms or reschedules from `/portal/sessions`.

    **Net effect:** the scattered path is self-enforcing. The uniform path is a soft trust risk — parents may not realize the live call needs confirmation and might miss it. Lesson materials still deliver, money is kept either way; it's a parent-experience gap, not a refund gap.

    **Uniform gap closed (2026-05-24).** `/portal` home now detects uniform-predicted slots (`live_call_at NOT NULL AND live_call_event_id IS NULL`) and shows a "New cycle" confirmation card. Parent can review sessions or dismiss with "Got it, remind me in Sessions" (stamps `uniform_schedule_acknowledged_at`). `/portal/sessions` `ActiveCycleManager` shows predicted slots in rare-blue with a "Confirm booking" primary button that opens a `ConfirmPredictedModal` — same Calendly embed flow, posts to `/api/portal/sessions/[id]/confirm-predicted` to write the real event URI without touching skip counters. No Calendly auto-booking from the server side (Calendly's API doesn't support programmatic invitee creation); the auto-suggest UI approach is the correct implementation.

    Original spec said "monthly curriculum approval" — that framing is preserved in the trust narrative (the parent IS in control: scattered ones via active scheduling, uniform ones via confirm/reschedule). Just no explicit "Approve" button each cycle. Lock this resolution before promising any new parent that "every cycle is opt-in" with a click — what we deliver is "every cycle is opt-in via scheduling friction."
...
7. ~~**Refund window enforcement.**~~ **DONE 2026-05-25.** Parent submits refund requests from `/portal/billing`; hard 60-day block in `/api/portal/refund/request` refuses outside the window. Peter reviews from `/admin/money` + Focused Home (`refund_request_pending` task, P77). Approve fires `stripe.refunds.create` with idempotency_key; Deny requires a note that lands in the parent email. See the Done entry for full surface.
...
6. ~~**Custom editor** (phase 2)~~ **REPLACED 2026-05-24.** The in-app 7-step lesson planner at `/admin/lessons/[id]/edit` is now the lesson editor. Video-first: Tim records the video externally, the planner produces the beat sheet + glossary + parent translation, published lesson rows hold `video_url`. Slides + audio model is gone. Original standalone HTML version of the planner lives at `tools/coach-keyed-planner/` as the design reference; the React port is the live surface. See the Path B Done entry for the full arc.
...
5. **Coaching server template.** Every operator runs their own Discord server, not a shared one. Platform provides a template config: required channels, required roles, required parent-observer setup. Operator cannot deviate without platform approval. (Recording bot removed — recording feature retired 2026-05-22.)
6. **Operator-kid welfare check.** If an operator kid shows signs of burnout, parent neglect, or behavior suggesting the parent isn't doing the work (parent unresponsive, kid taking too many clients, post-call notes thinning out, session quality degrading), platform escalates. Document the trigger thresholds and the protocol — probably starts with a private check-in call with the parent.
...
9. **Written operator agreement.** Contract every operator parent signs. Includes Hard Rules verbatim, code of conduct, message + coach-note retention policy, incident reporting obligations, indemnification, termination conditions, dispute resolution. Lawyer review (see #19). (Recording retention policy dropped — recording retired 2026-05-22.)
10. **90-day supervised trial period.** Every new operator runs 2–3 clients for 90 days with closer platform oversight: weekly check-in call, platform sits in on a sample of live sessions via parent-observer access, platform reviews coach notes + chat threads, platform shadow-replies to client-parent escalations. Operator graduates to full autonomy only after a clean 90 days. (Recorded-session review dropped — recording retired 2026-05-22; live observation is the replacement signal.)
...
- _All known real product gaps are shipped. Add as they surface._

#### New feature requests

- **$24 single coaching session — smoke test passed 2026-05-25.** Phase 1 + Tim-side surfacing + Phase 2 (parent-portal UX + scheduler + Stage C decline offer) + schedule-before-pay restructure all shipped 2026-05-23 (see Done entries). End-to-end validated against fresh email: webhook → magic link → /portal single-session dashboard → /portal/sessions Calendly booking → confirmation email all clean. Stage C decline-with-single-session offer also exercised. Open polish:
  - **In-portal reschedule modal for single-session** (Phase 2.5 polish). Today single-session reschedule routes through Calendly's confirmation email link. Cycle reschedule has a proper in-portal modal; single-session can adopt the same pattern if friction shows up.
  - ~~**Deno-side Calendly REST cancel in `cron-payment-abandonment`.**~~ **DONE 2026-05-24.** New shared helper `supabase/functions/_shared/calendly.ts` exports `cancelCalendlyEvent` (mirrors the Node helper) and `cancelCurriculumEvents` (fetches all booked slot event IDs for a curriculum and cancels each). Wired into `cron-payment-abandonment` (24h expiry path — cancels before nulling slots) and `cron-scheduling-abandonment` (7d expiry — both the paid-renewal and initial-onboarding branches now cancel any partially-booked Calendly events before clearing slot records). Non-fatal: logs and continues if Calendly returns an error so the DB record is always cleared.
- **Tim's lesson production (10 lessons before first paying customer)** — Peter driving with Tim; Peter QC's as Tim ships. See `tim-lesson-requirements.md` at repo root for the Tim-facing brief. Tooling: **the in-app planner at `/admin/lessons/new`** (which redirects to `/admin/lessons/[id]/edit`) — full 7-step authoring with AI helpers, autosave, preview, library, bundles. Original HTML tool at `tools/coach-keyed-planner/` is now the design reference, not the daily-use surface.
...
#### Transcribe-from-video v2 (Step 1 shipped 2026-05-24, v2 ffmpeg pipeline shipped 2026-05-25)

- **Async job queue / background transcription**. Today the request blocks for up to 5 min (maxDuration=300s). Fine for realistic coaching clips; a polling-based status flow would improve UX for very long recordings.
...
- **60-day refund window enforcement shipped (2026-05-25).** Closes "What's NOT built" #7. Parent self-service refund-request surface on `/portal/billing` + Peter-reviews path on `/admin/money` + Focused Home. Hard block at the form: outside the 60-day window, the API refuses with 403 and the UI doesn't expose ineligible charges. No auto-refund on parent click — Peter approves each one personally. Operator-pair / multi-tenant Connect is deferred per the existing pressure-test plan; this lives in the platform Stripe (Peter's account) until Connect lands.
  - **Migration `20260525000300_refund_requests.sql`.** New `refund_requests` table: family_id + subscription_id FKs, requested_by_parent_id, stripe_payment_intent_id, amount_cents, charge_date (stamped at request time so the 60d math is auditable without re-fetching Stripe), reason, status enum (`pending` / `approved` / `denied`), decided_by_coach_id, decided_at, decision_note, stripe_refund_id, created_at + updated_at trigger. Partial unique index on `stripe_payment_intent_id WHERE status IN ('pending', 'approved')` prevents double-submission against the same charge (denied requests can be re-submitted). RLS: `refund_requests_coach_all` (Peter has full CRUD), `refund_requests_parent_select` (family-scoped read), `refund_requests_parent_insert` (WITH CHECK family_id = family_id_for_user() + requested_by_parent_id matches auth + status='pending'; the 60-day window itself is enforced in the API route, not RLS).
  - **`derived_tasks_view` extended** in the same migration. New `refund_request_pending` branch at **priority 77** — between `call_outcome_pending` (P78) and `single_session_needs_lesson` (P76). Refund + money + parent-waiting is real-time urgent; lands just below post-call ambiguity. Payload carries `amount_cents`, `charge_date`, `reason` so Focused Home can render the full context (kid name, dollar amount, "Charge was Xd ago", parent's quoted reason).
  - **`/api/portal/refund/request`** (parent POST). Z-validates `{payment_intent_id, reason}`. Validation order (cheapest to costliest): auth → body → DB (no existing open request) → Stripe (PI exists + status='succeeded' + customer matches family + created within 60 days + not already refunded) → insert. Outside-window returns 403 with code `outside_window` so the parent UI can show specific copy. Fires the "we got your request" branded email through `sendBrandedEmail` with `notification_log` audit.
  - **`/api/admin/refund/[id]/approve`** (coach POST). Optional `decision_note`. Fires `stripe.refunds.create({payment_intent, reason: 'requested_by_customer', metadata: {refund_request_id, family_id, subscription_id}})` with **`idempotencyKey: 'refund:<refund_request_id>'`** so a network blip + retry doesn't double-refund. Stamps `status='approved' + decided_by_coach_id + decided_at + decision_note + stripe_refund_id`. Sends the approved email to the parent. Special error path `db_update_failed_after_refund` surfaces with the Stripe refund id if the DB write fails after the actual refund went through, so Peter can hand-reconcile without losing the audit trail.
  - **`/api/admin/refund/[id]/deny`** (coach POST). `decision_note` is REQUIRED (vs optional on approve) because the parent needs to know why. Stamps `status='denied' + decision_note` and emails the parent with the note inline in a lime-bordered callout.
  - **`/portal/billing` UI** — new "Refunds" card below the cancellation section (component `RefundsSection` + `refunds.module.css`). Three sub-sections, each only renders when non-empty: **Eligible charges** (Stripe query for the family's PIs in the last 60 days, filtered to succeeded + not yet refunded + no existing refund_request; each row shows "$X . [label] . Charged [date]" + "[N] days left in refund window" + a **Request refund** button that expands an inline reason textarea); **Awaiting decision** (rows with status='pending', amber "Pending . Requested [date]" pill + the parent's own reason quoted back to them); **Past requests** (resolved history — green "Refunded" or grey "Declined" pill + Peter's decision note when present). Server-rendered initial state; client-side `useRouter().refresh()` on successful submit so the new pending row appears immediately.
  - **`/admin/money` UI** — new "Refund requests" card (component `RefundReviewSection` + `refunds.module.css`). Header shows pending count pill ("3 pending"). Each pending row: kid first name + parent's family + dollar amount + charge date + "requested Xd ago" + parent's quoted reason in a lime-bordered callout. Per-row controls: **Approve** (optional note textarea + lime "Refund $X" button) / **Deny** (required note textarea + red "Send denial" button) / **Open client card** (ghost link). After action, the row collapses to a small "Refunded. Email sent." confirmation while `router.refresh()` re-fetches. Past requests appear in a compact history list below with status pill + decision note. Headline stats grid also got a "Refunds pending" tile (only renders when >0). "Coming later" bullets had "Refund history (deferred...)" removed since it's now built.
  - **Focused Home (`AdminClient.tsx`)** — `refund_request_pending` branch added. Uses the amber `pastDuePill` styling + amber `focusedHomePastDue` body variant + the `focusedHomeTitleLarge` heading style (treated as action-required + money-urgent, same visual weight as past-due). Title: `"[Kid]'s family asked for a refund."`. Body: dollar amount + "Charge was Xd ago" + parent's reason quoted (clipped at 160 chars). Action row: primary **Review refund** → `/admin/money` + secondary **Open client card** → `/admin/clients?client=<id>`. Stuck button still works (falls through to the generic `'other'` object_type mapping).
  - **Branded emails (`src/lib/refunds/emails.ts`)** — three senders, all dash-free per Hard rule #8, all routed through `sendBrandedEmail` so `notification_log` gets the audit row. `EmailTrigger` enum extended with `refund_request_received` / `refund_request_approved` / `refund_request_denied`. `EntityType` extended with `refund_request` so the audit rows link back to the source row cleanly. Copy framing: "Peter (Tim's dad, who runs the back end) reviews every refund personally" + 24-hour response promise on the received email; "Refund usually appears on your statement in 5 to 10 business days" + "Stripe handles the actual move; we don't see the timing on our end" on approved; denied email leads with Peter's decision_note in a lime-bordered callout and points the parent at the in-app message channel for follow-up.
  - **TS posture:** `refund_requests` table isn't in `src/types/db.ts` yet (Docker wasn't running locally so no `npm run gen:types` could fire). Every read/write uses the existing `.from("refund_requests" as never)` pattern from the lesson_bundles precedent, plus a structural `as unknown as {...}` cast around the `.insert(...).select(...).single()` and `.update(...).eq(...)` chains to keep them callable. `npx tsc --noEmit` clean. Tighten back to typed literals after the next `npm run gen:types`.
  - **Override path preserved:** the ToS says 60 days; the system enforces 60 days. If Peter wants to be generous on a one-off (medical emergency, family situation), he can go to the Stripe dashboard directly and issue a refund there — Stripe lets you refund up to 120 days after a charge without paying the fee back. The in-app flow is the structurally-clean path; the dashboard escape hatch is documented but not surfaced.

- **Calendar Day / Week / Month views shipped (2026-05-24).** `/admin/calendar` was list-only with a placeholder "Day, week, and month views are next." Now 4 views with a toggle. List view behavior preserved as default.
  - **Toolbar** at the top of the page: 4 view-toggle pills (List / Day / Week / Month) + date nav (Prev / Today / Next + focal-date label) when in a grid view. List view has no focal date so its nav hides.
  - **Day view** — single column, 8am–10pm hour grid (matches Tim's availability windows + comfortable buffer), events absolutely positioned by time, red "now" line that re-renders every minute via `setInterval`. "No calls this day" empty state.
  - **Week view** — 7 columns (Sunday start, US convention) × hour grid. Day-of-week + date headers above each column; today's column gets lime underline + lime date number + the now line.
  - **Month view** — 6 rows × 7 cols (always 6 to handle months that span 6 weeks). Days outside the focal month dim to 55% opacity on `--bg-2`; today's cell tinted lime; up to 3 event chips per cell with "+N more" overflow indicator. Chips are tiny — time + kid first name only — clickable to open the modal.
  - **Past events** rendered at 55% opacity across grid views (delivered slots now stay visible as historical record — page.tsx widened from "today onward" to **−90 / +180 days** so grid navigation has past + future cells to render into).
  - **Cancelled events** strikethrough in all views (carries over from list view's existing styling).
  - **Color coding** consistent across views via shared `gridEvent*` / `monthChip*` classes: lime = paid lesson, rare-blue = VOD review, amber = trial call, muted = cancelled.
  - **Same modal opens from any view.** `EventModal` + `CoachCancelForm` + `OutcomeForm` (all from Round 1+2) stay unchanged. Each view just emits "user clicked this event"; modal is rendered at the top level.
  - **State posture: local React only, no URL params + no DB persistence.** Defaults to List on every page load; focal date defaults to today. Switching views mid-session preserves the focal date (jump from Week of June 15 → Day on June 17 → Month of June → Week of June 15 works naturally). If deeplinkable URLs (`?view=week&date=2026-05-30`) or cross-device persistence become wanted, both are small additive changes.
  - **Data layer widening** — `page.tsx` fetch window for `curriculum_slots.live_call_at` + `subscriptions.trial_call_at` changed from `gte(start_of_today)` to `[−90 days, +180 days]`. Also dropped the `delivered_at IS NULL` filter on slots so past delivered calls render as historical record in grid views. `CalendarEvent` type gained a `delivered_at` field (always null for trial). List view client-filters back to "future + cancelled past only" so its behavior is identical to Round 1.
  - **Mobile** — toolbar wraps; week + month allow horizontal scroll on narrow viewports; chips shrink. Day view is naturally column-friendly. Per Hard rule #7 (mobile-first across all surfaces).
  - **File layout**: all 4 views + helpers inline in `CalendarClient.tsx` (1251 lines total now). Considered extraction to sibling files but kept inline since views share heavy formatter + helper code; future split is mechanical.

- **Transcribe-from-video v2: ffmpeg pipeline (2026-05-25).** Closes the real-world failure where Tim uploads ProRes/uncompressed MOV files (500MB+ for a 3-minute clip) that exceeded the old 25MB Supabase Storage limit. The flow is now completely invisible to Tim: drag any video format → upload progress bar via XHR → "Preparing video..." → transcript lands.
  - **Architecture change:** eliminated Supabase Storage as the transcription intermediary. The video goes directly from the browser to the transcribe route as `multipart/form-data`. `busboy` streams the upload to a temp file on disk (no memory buffering — safe for 500MB+ files). `ffmpeg-static` (bundles the ffmpeg binary for linux-x64/macOS; no `nixpacks.toml` needed) extracts audio-only at 64kbps mono 16kHz MP3. A 3-minute ProRes clip at ~500MB becomes a ~1.5MB MP3. Whisper receives the MP3; the 25MB cap is irrelevant in practice. If the audio somehow still exceeds 24MB (very long recording), the route retries ffmpeg at 32kbps. Both temp files are always deleted in the `finally` block.
  - **Client changes:** removed the 25MB client-side guard. Removed the Supabase Storage upload step. Now uses `XMLHttpRequest` (not fetch) so `xhr.upload.onprogress` gives a real percentage counter. Phases are now `uploading X%` → `processing (Preparing video...)` → `ready`. The "retry" concept is gone since there's no stored file — retry means drag again.
  - **Server changes:** `route.ts` now accepts `multipart/form-data` (Busboy + `Readable.fromWeb`). ffmpeg extraction runs in the route. `maxDuration` bumped from 120s to 300s. `whisper.ts` simplified: the `.mov` MIME repackaging logic from v1 is gone (ffmpeg normalizes everything before Whisper sees it); always receives `audio/mpeg`.
  - **UI copy:** drop zone says "MP4, MOV, WebM and more. We auto transcribe with Whisper." No mention of size limits. "Upload failed" renamed to "Could not transcribe" since there's only one failure mode now.
...
- **Path B lesson planner shipped + polished + bug-hunted across the stack (2026-05-23 → 05-24).** Tim's lesson production now flows through the in-app 7-step planner. The Coach Keyed Lesson Planner (originally a standalone HTML tool at `tools/coach-keyed-planner/`) is now the live `/admin/lessons/[id]/edit` surface with autosave, AI helpers, video-first delivery, and dedicated student-facing preview. Big arc — collapsed here so future sessions can read the full thread.
  - **Migrations** (`20260524000000_lessons_video_planner.sql`, `20260524000100_lesson_series.sql`, `20260524000200_lesson_bundles.sql`):
    - Loosens NOT NULLs on the slide-era required columns (slides, fortnite_label, parent_label, parent_skill_description, topic, difficulty_level, duration_minutes, parent_talking_points) so a fresh planner draft can land at Step 1 with only `title + author_id`.
    - Adds video-first columns: `video_url TEXT`, `beat_sheet JSONB`, `terms JSONB`, `planner_state JSONB`. Sunday cron reads beat_sheet + terms + video_url; planner_state is editor-only (rough draft, watch notes, identify list, dependency flags, review checks, current step).
    - Adds Capstone series binding: `series_id UUID REFERENCES lessons(id) ON DELETE SET NULL`, `series_position SMALLINT`. All series members share `series_id = capstone.id`. Filtered index for fast lookups.
    - Adds bundle binding: new `lesson_bundles` table (title, description, author_id, is_published) + `lessons.bundle_id` FK + `bundle_position`. Single bundle per lesson for v1. RLS coach-only.
  - **API surfaces:**
    - `POST /api/admin/lessons` creates a fresh draft + seeds placeholder values so the insert succeeds whether or not the migration has landed (backwards-compat hardening after Peter hit silent fail on first deploy).
    - `PATCH /api/admin/lessons/[id]` partial update of all planner fields. `DELETE` removes a draft, refuses with 409 if any `curriculum_slot` references it.
    - `POST /api/admin/lessons/ai-suggest` extended with 4 new `kind` values: `read_summary` (Step 2 — summarize transcript), `identify_breakdown` (Step 3 — break into teaching points), `narrow_recommend` (Step 4 — order skills), `write_structure` (Step 5 — draft beat sheet). Separate `PLANNER_SYSTEM_PROMPT` lets the AI use Fortnite vocabulary freely while keeping the no-dashes rule.
    - `POST /api/admin/lessons/[id]/spawn-series` (Capstone Mode) and `POST /api/admin/lessons/[id]/spawn-independent` (Standard Mode) — each creates N child lessons from the planner's identify list. Capstone binds via series_id; Standard creates independent lessons.
    - `POST /api/admin/lesson-bundles`, `PATCH/DELETE /[id]`, `PUT /[id]/lessons` — full bundle CRUD.
  - **Planner UI (`/admin/lessons/[id]/edit`):** React port of the standalone HTML tool. 7 steps (Rough draft → Read → Identify → Narrow → Write → Review → Publish) with debounced autosave (250ms PATCH), AI helpers at every step except Review, `sendBeacon` flush on unmount. AI output drops into editable fields; never auto-overwrites. Step 7 has its own AI helper for `parent_translation` + a "Copy outline" button that writes a plain-text dump of the beat sheet to clipboard. Step 7 also pre-fills the Fortnite term from the narrowChoice (no redundant typing). **AI suggestions rewritten in first-person singular** ("I'm teaching..." not "Tim is teaching...") per Peter's feedback — output reads as Tim's voice for direct paste into the beat sheet.
  - **Mode distinction (Step 3 + 4):** dependency question rewritten as two big descriptive cards (Independent vs Sequential), not equal-weight buttons. Standard Mode now has a real "Spawn N independent lessons" button parallel to Capstone's spawn — but creates lessons WITHOUT series_id binding. Library badges differ: Capstone series gets `PART X/N` + blue left border + `CAPSTONE` on the finale; Standard spawns are plain rows. Step 5 in Capstone mode shows "Capstone synthesis" eyebrow + copy telling Tim to reference (not re-teach) the foundation lessons.
  - **Library list (`/admin/lessons`):** restructured for the video-first product. Tab nav (Published / Drafts / Bundles), default = Published. Each row: title (ellipsis-truncated on long), Fortnite-label → parent-label sub, status badges (PUBLISHED / DRAFT / NEEDS VIDEO + topic/difficulty/duration + series/bundle), kebab menu (`⋮`) per row with Preview as student / Edit / Delete. Delete uses the existing endpoint's 409 refusal when curriculum_slot references the lesson. Published rows' inline link is "View as student" → preview; draft rows' inline link is "Open planner" → edit.
  - **Student-facing preview (`/admin/lessons/[id]/preview`):** dedicated route, NOT the planner. Toggleable kid/parent view. Kid view: Fortnite title, video embed, numbered key beats with Why subtext, glossary, common mistake / practice / recap / outro. Parent view: real-world skill label first with Fortnite term in parens (Hard rule #4), parent_skill_description as lead, 🤫 For your back pocket talking-point cards, glossary at the bottom. Section spacing pulled in aggressively after Peter's screenshots showed 80-100px gaps (`.lessonView gap: 6px`, smaller beat boxes, tighter talking points). YouTube/Vimeo/Loom URLs auto-embed via iframe; raw URLs use `<video>` fallback; unrecognized falls back to "Watch on [domain] →" link.
  - **Sunday cron updated** to ship a video-link email instead of slide+audio attachments. Skips lessons without `video_url + is_published` with new skip reason `lesson_missing_video`. Translation pair + back-pocket talking points block stay intact in the email.
  - **Bundles tab** (the third tab in /admin/lessons): list of bundles with create / rename / delete inline. Each row expands to a membership editor — pick lessons from the unbundled pool, reorder ↑↓, remove ×, save. `PUT /[bundle_id]/lessons` does whole-list replace atomically. Bundles are internal organization for v1 (not yet surfaced on parent/kid views); a lesson can be in both a Capstone series AND a bundle.
  - **Shared LessonPicker component (`src/components/LessonPicker.tsx`)** wherever Tim picks a lesson. Replaces the divergent substring-only inputs in the swap modal + the 20-row fallback in the bundles tab. Fuzzy search via 40-line Levenshtein-aware scorer at `src/lib/lessons/fuzzy.ts` (no Fuse.js dep). Lightweight filter chips: Topic / Difficulty dropdowns, Published-only / Drafts-only / Unbundled / Not-yet-taught toggles, Recent-first sort, Clear filters. Result rows show all the structure-at-a-glance badges Peter asked for (published/draft, already taught, capstone/part/standalone, bundle name, topic, difficulty, duration).
  - **Single-session deliverability bug** (Peter's diagnosis: "subscription delivery logic was the only path that surfaced a lesson to the kid"): root-caused to the swap-lesson endpoint setting `lesson_id` but never `delivered_at` — that's a Sunday-cron-only signal, but the kid view reads `delivered_at` for visibility. Fixed in two places. (1) Swap-lesson endpoint now detects single-session curricula and stamps `delivered_at = NOW()` at assignment time. (2) /play main view + /play/library both branch on subscription `tier` — single-session gets its own "Tim assigned a lesson" card with the Watch link, no "Lesson 1 of 4 incoming Sunday" cycle leakage. The mental-model split is now real in code: subscription = cron-progressed, single-session = immediate-on-assign.
  - **UTC vs PT timezone bug** (Peter's screenshot: 4:30pm PT booking shown as "11:30pm" on /portal): every Server-Component formatter in /portal/* called `Intl.DateTimeFormat` without an explicit `timeZone` — server defaults to UTC on Railway. Fixed via new `src/lib/datetime.ts` with PT-pinned formatters (`formatCallDateTime`, `formatTime`, `formatShortDate`, `formatLongDate`, `formatCompactDate`). Time values now show `"4:30pm PT"` with PT suffix so cross-timezone parents can't misread. Swept all /portal Server Components + the single-session paid email. Admin + /play formatters NOT swept this pass (Tim's browser is on PT, lower blast radius).
  - **"Send now" override for cycle subs** — Peter flagged as a wanted feature, then noted "I think this is already solved." Confirmed: cycle subs already have 3 paths that set `delivered_at` (Sunday cron, Stripe webhook on checkout for Lesson 1 via `deliver-week-one.ts`, Tim marking the call outcome via `/api/admin/calendar/mark-outcome` with `outcome='done'`). The mark-outcome path IS Tim's manual override — naturally tied to the call-ended moment, not a separate button. **No new code needed.**
  - **Stripe refund-fee question** — informational. Stripe's 2023+ policy: full refunds do NOT return the processing fee on refunds for US merchants. $24 single → ~$1 fee lost on test+refund cycle. $56 cycle → ~$1.92 lost. Recommendation: use Stripe Test Mode (`sk_test_*`, `pk_test_*`, card `4242 4242 4242 4242`) for testing. No code change.

- **Single-session schedule-before-pay restructure (2026-05-23).** Closed the "what if I pay and no time works" anxiety Peter caught while testing. Single-session was the only paid surface in the codebase that asked for money before showing availability; now it mirrors the cycle flow (collect details, pick a time, then pay). Commit `2c15f91`.
  - **Form L4 became a multi-stage final boss without expanding the 4-level rarity progression.** Stage machine extends from `{form, submitting, submit_failed}` to also include `scheduling`, `ready_to_pay`, `opening_stripe`, `pay_failed`. The same L4 card morphs through three sub-renderings: "What to help with" textarea + "Continue to scheduling" CTA → Calendly embed (paid-lesson event type, prefilled from form state) → "Time held. $24 to lock it in." pay CTA. XP bar stays at L4=95% throughout; gamification rhythm preserved.
  - **`/api/single-session/submit`** no longer creates Stripe Checkout. Creates account + curriculum + slot, parks subscription at `lifecycle_state='ACCEPTED_PENDING_SCHEDULING'` with `payment_pending_at=NULL`, returns `{ ok, subscription_id }`. Stripe Customer object IS created here (idempotent, cheap, sets `families.stripe_customer_id` for the second-phase endpoint).
  - **New `/api/single-session/checkout`** mints a fresh Stripe Checkout Session against the existing customer + curriculum metadata. Called from the form's pay button after Calendly fires. Idempotent at the Stripe level: each retry creates a new session, old ones expire naturally on Stripe's side.
  - **Calendly webhook's single-session branch rewritten** (was: flip lifecycle to `ACTIVE` on booking + send confirmation email — wrong for pre-pay). Now: flips lifecycle to `PENDING_PAYMENT` + stamps `payment_pending_at=NOW()` so the existing `cron-payment-abandonment` cron picks it up. **No branded email sent** at this stage — sending a "time is held" email before payment confuses the parent. Confirmation lands post-payment via `sendSingleSessionPaidEmail`.
  - **Stripe webhook `activateSingleSessionAfterPayment` rewritten.** Was flipping lifecycle to `SCHEDULING_IN_PROGRESS + waiting_on=PARENT` and sending a "now schedule" email (correct under the old pay-first order; wrong now). New behavior: lifecycle → `ACTIVE`, `waiting_on='TIM'` (next event is the call itself), `payment_pending_at=NULL`. Post-payment email rewritten to "session is locked in for `<date>`" with `next=/portal` CTA (no more scheduling needed). Slot's `live_call_at` is fetched and rendered inline in the email.
  - **Success page copy refreshed.** "Session Booked" → "Session Locked In", dashboard-open replaces scheduler-open, the "what happens next" ordered list drops the "pick a time" step.
  - **Mobile hamburger** picked up the Sign in link (was already in desktop nav, missing in the mobile drawer).
  - **Known v1 tradeoff:** if the parent abandons between scheduling and pay, `cron-payment-abandonment` clears slot data after 24hr but does NOT call Calendly's REST API to cancel the event itself. Tim sees a phantom event in Calendly with no DB record and manually cancels. Captured as Phase 2.5 follow-up in the Open TODO above (~30 min Deno-side fix when annoyance kicks in).

- **Single-session Phase 2 shipped: parent portal UX + Stage C decline offer (2026-05-23).** Three commits closing the polish gaps from the original Phase 1 ship.
  - **`/portal` dashboard** branched on `subscription.tier`. Single-session families landed on the cycle-subscriber UI (Lesson 0 of 4, Sunday lesson drop, 2 cancellations) because `phaseFor()` only keyed off `status`. Now `/portal/page.tsx` fetches `tier` + curriculum + slot when `tier='single_lesson'` and renders new `LiveSingleSessionCards` instead of `LiveSummaryCards`. Hero copy switches to "Single session" with three states: pick a time / scheduled on <date> / session complete. Three new cards: Coaching session, What we're working on (parent's intake_note + lesson Tim picked once assigned), Messages. Same 5s polling + just-updated highlight pattern. Cycle-only alerts (past_due, pending_cancel, ended) are mutually exclusive by branch since single-session subs land status='active' from the Stripe webhook and never enter those states.
  - **`/portal/sessions` scheduler** rebuilt as `SingleSessionScheduler` for `tier='single_lesson'`. Calendly embed when unscheduled; "Session is set / was / complete" card when populated. No cycle anchor, no skip counter, no Repeat-at-this-time shortcut. Parent's intake_note surfaces above the embed as a soft callout so they see what they told Tim while picking the time. Reschedule v1 points at the Calendly confirmation email's reschedule link (the in-portal reschedule modal can come later if friction shows up). All four cycle code paths (SchedulerWizard / PaymentSummary / ActiveCycleManager / "Not ready yet") explicitly gate on `!isSingleSession` so they never collide.
  - **Calendly webhook `handlePaidLessonCreated`** broadened. Was `status='pending_approval'` only — single-session curricula flip to 'active' the moment Stripe settles, so the lookup returned no rows and the slot write was silently no-op'd. Now `status IN ('pending_approval', 'active')` + fetches `curriculum_type`; a single-session branch skips the cycle_anchor_at write + skips the PENDING_PAYMENT lifecycle transition + sends a single-session-specific confirmation email without "Week N of N" framing. After single-session booking, subscription flips to `lifecycle_state='ACTIVE'` + `waiting_on='TIM'`.
  - **Stage C decline email** now includes the $24 single-session CTA as the middle rung of the downgrade ladder. Tim's "Not the right fit" decision used to jump straight from "monthly cycle isn't right" to free creator recommendations; the new copy framing reads honest-not-salesy ("monthly isn't the fit for [kid] right now... if you want a one-off check-in instead, $24 single session") with the free recs preserved below as the no-paid alternative.
  - **Three commits**: `45f8a9e` (dashboard), `c755f64` (scheduler + webhook), `8eb6627` (decline email). Each independently reviewable. `npx tsc --noEmit` clean.

- **$24 single coaching session shipped end to end (2026-05-23).** Net-new product alongside the $56/mo cycle subscription. Designed + priced 2026-05-22, built + bug-hunted through 2026-05-23. Tim's n=1 instance now sells two tiers from one codebase.
  - **Pricing + framing locked.** $24 single = $14 per-cycle-lesson + $10 no-commitment premium. Preserves cycle economics + makes the "subscription = primary" mental model concrete. Marketing pricing card on `/` shows the saving directly ("Best Value · Save 42%" badge, cycle-line reads "just $14 each vs $24 single"). Naming: **always "single coaching session"** in user-facing copy, never "single lesson" — the internal DB enum stays `tier='single_lesson'` (a forward-built schema value from the original spec) but no user surface sees that string.
  - **Who can buy:** trial families (decline path alternative — Phase 2) + marketing-site cold traffic. Active subscribers explicitly excluded in v1.
  - **Lesson selection is Tim-side, not parent-side.** Original locked design 2026-05-22 had the parent picking from a published catalog; reversed 2026-05-23 after Peter's review. Parent provides intake + free-text "what they want help with"; Tim picks (or builds) the lesson post-payment based on that signal. Matches the cycle model (Tim is always the curator) and removes Tim's decision fatigue at sales time.
  - **Form UX:** 4-level gamified flow mirroring `/intake` (XP bar, rarity colors per level, +25 XP floats, sound toggle, level transitions). Reuses `/intake`'s CSS module wholesale; slim local `/single-session/page.module.css` adds the L4 pay-block + L3 locked-parent display. Level structure: L1 Player Profile (uncommon green) → L2 Skill Check (rare blue) → L3 Parent Contact (epic red, or locked-already-verified for under-13) → L4 What & Pay (legendary orange). Inline COPPA gate at L1 for under-13 reuses the existing `/api/intake/request-verification` + `/intake/verify` infrastructure (the verify route's `return_to` allow-list was extended to include `/single-session`).
  - **Success screen** matches `/intake`'s SuccessCard celebration: ACHIEVEMENT UNLOCKED kicker, pulse-animated "Session Booked" headline, confetti burst (lazy-imported `canvas-confetti`), success chime if sound was on during the form. Server Component shell + Client Component for the audio/confetti. Personalized "We emailed you at <address>" line via Stripe Checkout `success_url={CHECKOUT_SESSION_ID}` lookup → `customer_details.email` extraction. Email never appears in the URL.
  - **Data model:** new `curriculum_type` enum (`'subscription' | 'single_session'`) on `curricula` via migration `20260522000200_curriculum_type.sql`. Slot reuses everything: `curriculum_slots`, scheduling, Sunday delivery, `coach_note`, reschedule, Stripe Checkout. Auto-renew cron + cycle-skip math naturally skip (we set `auto_renew_enabled=FALSE` + `auto_renew_off_acknowledged_at=NOW()` at creation; `cycle_started_at` stays NULL so `cycle_drag_out` doesn't fire). Slot `lesson_id` starts NULL — Tim assigns post-payment via the existing `/admin/clients` lesson-swap UI.
  - **Account creation:** real family/parent/player rows + Supabase auth users for both parent and synthetic kid. Parent gets `/portal` access, kid gets `/play` access. Same RLS/auth as cycle subscribers — just 1 slot instead of 4 and no auto-renew. Lesson materials remain accessible post-call; kid can still message Tim.
  - **Webhook activation:** `checkout.session.completed` → `metadata.kind='single_session'` → new `activateSingleSessionAfterPayment` branch in `src/app/api/stripe-webhook/route.ts`. Flips curriculum to active + subscription to `status='active' + lifecycle_state='SCHEDULING_IN_PROGRESS'`. Sends a magic-link "your session is paid, schedule the call now" email via the new `sendSingleSessionPaidEmail` helper.
  - **Tim-side notification.** New `derived_tasks_view` task type `single_session_needs_lesson` (priority 76, between `call_outcome_pending` P78 and `lesson_authoring_needed` P75). Fires when a single-session subscription is active + the slot has `lesson_id=NULL`. Task body quotes the parent's intake note inline so Tim has the signal he needs without leaving Focused Home. Drops automatically when Tim assigns a lesson via the existing swap UI. Tim's existing **Dismiss** button works on this task type with no special handling.
  - **Three required-constraint migrations along the way:**
    1. `20260522000200_curriculum_type.sql` — adds the curriculum_type enum + column.
    2. `20260523000000_lesson_xor_vod_allow_tbd.sql` — relaxes the `lesson_xor_vod` CHECK constraint on `curriculum_slots` to allow a third "TBD" state (`is_vod_review=FALSE + lesson_id NULL + vod_url NULL`) since single-session slots are created without a lesson assignment.
    3. `20260523000100_single_session_needs_lesson_task.sql` — re-CREATE OR REPLACE `derived_tasks_view` with the new task branch added.
  - **Bug bash during live testing** (all fixed):
    1. **`subscription_insert_failed`** — submit endpoint inserted `tier='single'` but the existing CHECK constraint only allows `'trial' | 'monthly' | 'single_lesson'`. Fixed by using `'single_lesson'` (the forward-built schema value from the originally-specced $14 single lesson product).
    2. **`slot_insert_failed`** — `lesson_xor_vod` CHECK required either `lesson_id` or `vod_url` to be set; single-session slots have both NULL until Tim assigns. Fixed by relaxing the constraint (migration 20260523000000).
    3. **Magic-link email silently swallowed after payment** — `sendParentMagicLink` calls `.ilike("email").maybeSingle()`, which errors when 2+ rows match. The earlier failed submit attempts had left orphan parent rows (cleanup only deleted auth users, not the parent row), so a fresh successful submit's email lookup found 2 matching parents (one with `auth_user_id=NULL` from the orphan, one real). Fixed by adding `.not("auth_user_id", "is", null).order("created_at", desc).limit(1)` to both `sendParentMagicLink` and `sendPlayerMagicLink`. Plus hardened the submit cleanup to cascade-delete the family (which cascades through parents → players → subscriptions → curricula → curriculum_slots via the existing FK ON DELETE CASCADE chain) on any post-family-insert failure, preventing future orphans from accumulating.
    4. **Single-session email never reached Resend** — additional context: the email failure was completely silent because `sendSingleSessionPaidEmail` awaited `sendParentMagicLink` without checking the `{ok:false, code:...}` result. Hardened the caller to log both success and failure cases.
  - **Phase 2 still pending** (small): Stage C decline-with-single-session-offer flow (graceful downgrade ladder), and end-to-end smoke test with a fresh email to validate the now-fixed webhook → magic-link path.
...
1c. ~~**Turn off Calendly's invitee confirmation email**~~ **RESOLVED 2026-05-22.** No toggle exists in Standard tier to disable Calendly's invitee email (we looked). Instead we customized Calendly's email TEMPLATE (Notifications → Email Confirmation) to a short on-brand acknowledgment that defers detail to the branded email our `invitee.created` webhook sends. Parents now get two emails but they don't conflict — Calendly's is a brief "you're booked" + the branded one carries the Discord trust copy, kid name name-check, and full content. Acceptable cost vs. the alternative of upgrading to Calendly Teams just to find the toggle.
