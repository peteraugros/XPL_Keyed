...
6. **Don't over-build the editor.** Lessons are web-native (slides as JSON + per-slide audio MP3s), not .pptx files. During MVP, Tim makes lessons in Google Slides + records audio in QuickTime. **The originally-planned phase-2 custom in-browser editor is retired (2026-05-23) — replaced by the standalone Coach Keyed Lesson Planner at `tools/coach-keyed-planner/` (single-file HTML, Tim's local machine, separate from the deployed app). The planner produces beat sheets Tim records from; the existing `/admin/lessons/new` upload form remains the path for slides + audio into the lesson library.**
...
6. ~~**Custom editor** (phase 2)~~ **RETIRED 2026-05-23.** Replaced by the standalone Coach Keyed Lesson Planner at `tools/coach-keyed-planner/`. Different shape than the originally-imagined in-browser slide editor: the planner produces beat sheets Tim records video from, not slides Tim builds in-app. Slides + audio for the existing `/admin/lessons/new` library upload still come from Google Slides + QuickTime; the planner is upstream of that workflow, not a replacement for it.
...
- **Tim's lesson production (10 lessons before first paying customer)** — Peter driving with Tim; Peter QC's as Tim ships. See `tim-lesson-requirements.md` at repo root for the Tim-facing brief. Tooling: **Coach Keyed Lesson Planner** at `tools/coach-keyed-planner/coach-keyed-planner.html` (single-file HTML, double-click to open, localStorage-backed). Spec at `tools/coach-keyed-planner/spec-v1.1.md`. Replaces the originally-planned phase-2 in-browser editor (see Hard Rule #6 + Build Phases #6 above). Tone calibrated for 14yo. Open follow-ups: the five spec-appended items + lock the dependency-question wording after Tim's first real session.
