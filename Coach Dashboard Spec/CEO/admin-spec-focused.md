# XPL Keyed Admin: Focused Mode

The XPL Keyed admin ships in two modes. This is one of them.

**Focused mode** is the admin for operators who want the system to think 
for them — to surface the next right thing, scaffold the decision, and 
get out of the way. It optimizes for *will I actually do the work right 
now?* Designed first for Tim, a 14-year-old with ADHD running his first 
business alongside his dad. Useful for: ADHD operators, novice operators, 
anyone overwhelmed by lists, anyone learning the work, anyone in a 
low-energy window.

**Command mode** is the admin for operators who want to see the whole 
picture and decide for themselves. Lives in `admin-spec-command.md`. 
Optimizes for *can I see and move through everything efficiently?* 
Useful for: experienced operators, high-agency users, anyone in a 
focused work window who wants to batch-process, anyone who needs to 
find a specific thing fast.

**The toggle** between modes is per-user, persistent, and respected. 
Neither mode is "the real one" or "the advanced one." They are two valid 
operator mindsets for two valid moments. Mode-switching behavior is 
specified in `admin-modes.md`.

For Tim specifically, Focused mode is almost certainly his lifelong 
default — the responsibility curriculum lives in this mode, and the 
ADHD-friendly affordances are calibrated to him. For other operators, 
Command mode may be their default from day one.

This is not a SaaS admin tool. It is a product designed to help Tim 
become the operator he could be, by making it obvious, satisfying, and 
shame-free to do the next right thing.

The work this admin does is real. The responsibility it builds is the 
point.
