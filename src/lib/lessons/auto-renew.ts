...
export async function provisionNextCycle(
  ...
  // A renewal cycle creates BARE sessions. No lesson selection.
  //
  // This used to run a three step chain: 4 fresh published lessons, else top
  // up from lessons the player had already had, else mint stub rows so the
  // renewal loop completed while Tim authored content during the cycle. All
  // three steps existed to keep a library fed. There is no library, and the
  // task that chased Tim about the stubs (lesson_authoring_needed) was removed
  // from derived_tasks_view in the same phase.
  //
  // Note what is NOT lost: the renewal still creates the cycle, still predicts
  // uniform call times, still transitions the lifecycle, and still bills.
  // Only the content attachment is gone.
